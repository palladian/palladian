...
public final class TreeBuilder implements PredictiveModelBuilder<Tree> {
    ...
    public TreeBuilder minProbability(double minProbability) {
        this.minProbability = minProbability;
        return this;
    }

    public TreeBuilder attributeExcludeDepth(int depth) {
        this.attributeExcludeDepth = depth;
        return this;
    }

    public TreeBuilder excludeAttributes(Set<String> attributes) {
    ...
    protected Node buildTree(Node parent, final Iterable<? extends AbstractInstance> trainingData, final int depth, final Map<String, double[]> splits) {
        ...
        for (final Entry<String, AttributeCharacteristics> e : attributeCharacteristics.entrySet()) {
            ...
            if (thisPair == null || thisPair.getValue() == 0) {
                thisPair = createNominalNode(parent, e.getKey(), trainingData);
            }
            if (thisPair.getValue() > bestScore) {
                bestScore = thisPair.getValue();
                bestNode = thisPair.getKey();
            }
        }
    ...
    protected Pair<? extends Branch, Double> createNominalNode(Node parent, final String attribute, final Iterable<? extends AbstractInstance> instances) {
        ...
        ClassificationCounter outCounts = valueOutcomeCountsPair.getKey();
        final Map<Serializable, ClassificationCounter> valueOutcomeCounts = valueOutcomeCountsPair.getValue();
        ...
        return Pair.of(new NominalBranch(parent, attribute, bestSoFar), score);
    }
    ...
    protected Pair<? extends Branch, Double> createOrdinalNode(Node parent, final String attribute, final Iterable<? extends AbstractInstance> instances, final double[] splits) {
        ...
        return Pair.of(new OrdinalBranch(parent, attribute, bestThreshold), bestScore);
    }
