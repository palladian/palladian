...
public class KeywordAnnotator extends AbstractPipelineProcessor<String> {
	...
	public void processDocument(PipelineDocument<String> document) {
		...
		AnnotationFeature annotationFeature = (AnnotationFeature) featureVector
				.get(TokenizerInterface.PROVIDED_FEATURE);
		...
		for (Annotation annotation : tokenList) {
			...
			// double tfidf = ((NumericFeature)
			// tokenFeatureVector.get(TfIdfAnnotator.PROVIDED_FEATURE)).getValue();
			double df = ((NumericFeature) tokenFeatureVector
					.get(FrequencyCalculator.PROVIDED_FEATURE)).getValue();
			double spread = ((NumericFeature) tokenFeatureVector
					.get(TokenSpreadCalculator.PROVIDED_FEATURE)).getValue();
			double uppercaseScore = StringHelper.startsUppercase(annotation
					.getValue()) ? 1 : 0.5;
			double positionScore = 1. - (double) annotation.getStartPosition()
					/ docLenght;
			// double lengthScore = annotation.getValue().split(" ").length;

			double ranking = df * (spread + 1.0) * uppercaseScore
					* positionScore;
		...
		Feature<List<Keyword>> keywordFeature = new Feature<List<Keyword>>(
				PROVIDED_FEATURE, keywordList);
	...
	public static void main(String[] args)
		...
		PipelineDocument result = pipeline.process(new PipelineDocument(
				originalContent));

		Feature<List<Keyword>> feature = (Feature<List<Keyword>>) result
