...
public class ContentDateComparator implements Comparator<ContentDate> {
    ...
    public int compare(ContentDate date1, ContentDate date2) {
        ...
        return (date1.get(ContentDate.DATEPOS_IN_DOC) < date2.get(ContentDate.DATEPOS_IN_DOC) ? -1 : (date1.get(ContentDate.DATEPOS_IN_DOC) > date2.get(
                ContentDate.DATEPOS_IN_DOC) ? 1 : 0));
