...
public final class ChiSquaredFeatureRanker extends AbstractFeatureRanker {
     ...
     *         class.
     */
    public Map<String, Map<String, Double>> calculateChiSquareValues(Collection<? extends Instance> dataset) {
        Map<String, Map<String, Long>> termClassCorrelationMatrix = CollectionHelper.newHashMap();
        ...
        for (Instance instance : dataset) {
            // Set<Feature<?>> features = convertToSet(instance.getVector());
            // features.addAll(discretize(features, dataset));
            // Set<Feature<?>> features=discretize(features, dataset);
            FeatureVector features = discretization.discretize(instance.getVector(), dataset);
            LOGGER.trace(features.toString());
            for (VectorEntry<String, Value> value : features) {
                addCooccurence(value.key(), instance.getCategory(), termClassCorrelationMatrix);
            }
            Long count = classCounts.get(instance.getCategory());
            if (count == null) {
                count = 0L;
            }
            classCounts.put(instance.getCategory(), ++count);
...
//    }
     ...
     */
    private static void addCooccurence(String row, String column,
            Map<String, Map<String, Long>> correlationMatrix) {
