...
public class DictionaryClassifier extends TextClassifier {
	...
	public DictionaryClassifier() {
		...
		dictionary = new Dictionary(getDictionaryName(),
	...
	public void addToDictionary(TextInstance trainingDocument, int classType) {
		...
		for (Map.Entry<Term, Double> entry : trainingDocument
				.getWeightedTerms().entrySet()) {

			// get the first category only
			if (classType == ClassificationTypeSetting.SINGLE) {
				dictionary.updateWord(entry.getKey(), trainingDocument
						.getFirstRealCategory().getName(), entry.getValue());
			}

			// get all categories for hierarchical and tag mode
			else {
				for (Category realCategory : trainingDocument
						.getRealCategories()) {
					// System.out.println("update " + entry.getKey() + " " +
					// realCategory.getName());
					dictionary.updateWord(entry.getKey(),
							realCategory.getName(), entry.getValue());
				}
			}
		}
		...
		if (classificationTypeSetting.getClassificationType() == ClassificationTypeSetting.TAG
				&& classificationTypeSetting.getClassificationTypeTagSetting()
						.isUseCooccurrence()) {

			Term[] terms;
			if (trainingDocument.getRealCategories().size() == 1) {
				terms = new Term[2];
				terms[0] = new Term(trainingDocument.getRealCategories().get(0)
						.getName());
				terms[1] = new Term(trainingDocument.getRealCategories().get(0)
						.getName());
			} else {
		...
		LOGGER.debug("added to dictionary in " + DateHelper.getRuntime(t1)
				+ " (" + DateHelper.getRuntime(initTime) + "), "
	 ...
	 */
	private void saveDictionary(String path, boolean indexFirst,
		...
		dictionary.serialize(path + getDictionaryName() + ".gz", indexFirst,
				deleteIndexFirst);
	...
	public static DictionaryClassifier load(String classifierPath) {
		...
		classifier = (DictionaryClassifier) FileHelper
				.deserialize(classifierPath);
		if (classifier == null) {
			throw new ClassifierNotFoundException(
					"No classifier was found at path: " + classifierPath);
		}
		classifier.reset();
	...
	public void loadDictionary(int classType) {
		...
		if (dictionary == null) {
			String modelFilePath = getDictionaryPath() + getDictionaryName()
					+ ".gz";
	...
	private void trainWithInstance(UniversalInstance instance) {
		...
		Category knownCategory = categories.getCategoryByName(instance
				.getInstanceCategory().getName());
		if (knownCategory == null) {
			knownCategory = new Category(instance.getInstanceCategory()
					.getName());
		...
		TextInstance trainingDocument = preprocessDocument(instance
				.getTextFeature());
	...
	protected double calculateRelevance(CategoryEntry categoryEntry,
			Map.Entry<String, Double> map) {
		return 0.0;
	}
	...
	private synchronized TextInstance classify(TextInstance document,
			boolean loadDictionary, Set<String> possibleClasses) {
		...
		for (Category category : categories) {
			if (possibleClasses != null
					&& !possibleClasses.contains(category.getName())) {
				continue;
			}
		...
		for (Map.Entry<Term, Double> weightedTerm : document.getWeightedTerms()
				.entrySet()) {

			CategoryEntries dictionaryCategoryEntries = dictionary
					.get(weightedTerm.getKey());
			...
			if (!dictionaryCategoryEntries.isEmpty()) {
				...
				for (CategoryEntry categoryEntry : dictionaryCategoryEntries) {
					...
					CategoryEntry c = bestFitList
							.getCategoryEntry(categoryName);
					if (c == null) {
						continue;
					}
				...
				LOGGER.trace("the term \""
						+ weightedTerm.getKey().getText()
		...
		if (classificationTypeSetting.getClassificationType() == ClassificationTypeSetting.TAG
			...
			for (CategoryEntry c : document
					.getAssignedCategoryEntriesByRelevance(classType)) {
				numberOne = c;
				break;
			}
			...
			if (numberOne != null) {
				...
				List<WordCorrelation> correlations = dictionary.getWcm()
						.getCorrelations(numberOne.getCategory().getName(), 1);
				for (WordCorrelation wc : correlations) {
					String cooccuringCategory = wc.getTerm1().getText();
					if (cooccuringCategory.equalsIgnoreCase(numberOne
							.getCategory().getName())) {
						cooccuringCategory = wc.getTerm2().getText();
					}

					CategoryEntry cen = bestFitList
							.getCategoryEntry(cooccuringCategory);
		...
		if (classType == ClassificationTypeSetting.HIERARCHICAL) {
			...
			for (CategoryEntry c : bestFitList) {
				TreeNode tn = getDictionary().hierarchyRootNode.getNode(c
						.getCategory().getName());
				if (tn != null) {
			...
			try {
				...
				List<TreeNode> nodes = getDictionary().hierarchyRootNode
						.getNode(mc).getFullPath();
				for (TreeNode node : nodes) {
					if (node == null) {
						continue;
					}
					CategoryEntry ce = bestFitList.getCategoryEntry(node
							.getLabel());
					if (ce != null) {
				...
				Logger.getRootLogger()
						.error("class "
								+ mc
								+ " was not learned and could not be found in hierarchy tree: "
								+ document.getMainCategoryEntry().getCategory()
										.getName(), e);
		...
		// keep only top X categories for tagging mode
			...
			document.limitCategories(classificationTypeSetting
					.getClassificationTypeTagSetting().getMinTags(),
					classificationTypeSetting.getClassificationTypeTagSetting()
							.getMaxTags(), classificationTypeSetting
							.getClassificationTypeTagSetting()
		...
		// calculate one regression value for the given documents
			...
			for (CategoryEntry ce : document.getAssignedCategoryEntries()) {
				if (ce.getRelevance() > 0) {
					regressionValue += Double.valueOf(ce.getCategory()
							.getName()) * ce.getRelevance();
				}
			}
			document.getAssignedCategoryEntries().clear();
			document.addCategoryEntry(new CategoryEntry(document
					.getAssignedCategoryEntries(), new Category(String
		...
		if (document.getAssignedCategoryEntries().isEmpty()) {
			...
			CategoryEntry defaultCE = new CategoryEntry(bestFitList,
					unassignedCategory, 1);
			document.addCategoryEntry(defaultCE);
		}

		document.setClassifiedAs(classType);

		LOGGER.debug("classified document (classType " + classType + ") in "
	...
	public TextInstance preprocessDocument(String text,
			TextInstance classificationDocument) {
		PipelineDocument<String> processedDocument;
		try {
			processedDocument = processingPipeline
					.process(new PipelineDocument<String>(text));
		} catch (DocumentUnprocessableException e) {
			throw new IllegalArgumentException(e);
		}
		return preprocessor.preProcessDocument(
				processedDocument.getModifiedContent(), classificationDocument);
	}
	...
	public TextInstance preprocessDocument(String text) {
		...
		return preprocessor.preProcessDocument(processedDocument
				.getModifiedContent());
	}
	...
	/**
	 * <p>
	 * For quick thread-safe classification use this. The DictionaryClassifier
	 * is thread safe by itself but this is faster since copies of classifiers
	 * are created which all use the same dictionary (read-only).
	 * </p>
	 * 
	 * @param classifier
	 ...
	 */
	public static TextInstance classify(DictionaryClassifier classifier,
			String text) {
		...
		copy.setClassificationTypeSetting(classifier
	...
	public void threadTest() {
		...
		final DictionaryClassifier classifier = FileHelper
				.deserialize(classifierPath);
		...
		final DictionaryClassifierPool dc2Pool = new DictionaryClassifierPool(
				classifier, totalThreads);
		...
		for (int i = 0; i < totalThreads; i++) {
			...
			new Thread(new Runnable() {
				...
				public void run() {
					...
					TextInstance classify = dc2.classify(LoremIpsumGenerator
							.getRandomText(100));
					dc2Pool.release(dc2);
					// TextInstance classify =
					// dc2Pool.classify(LoremIpsumGenerator.getRandomText(100));
					System.out.println(sw2.getElapsedTimeString()
							+ " in current thread");
		...
		System.out.println("avg. time per thread: " + totalTime
				/ (double) totalThreads);
