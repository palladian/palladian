...
public class DummyVariableCreator extends AbstractDatasetFeatureVectorTransformer implements Serializable {
    ...
    private static final class Mapper {
        ...
        Mapper(Map<String, String> mapping, boolean dense) {
            ...
            featureInformation = new FeatureInformationBuilder().set(mapping.values(), ImmutableIntegerValue.class).create();
        }
        ...
        private static FeatureVector createDummyVector(FlyweightVectorSchema schema, String value, Collection<String> allValues, boolean dense) {
            if (dense) {
     ...
     *
     * @param dataset             The dataset, not <code>null</code>.
     * @param keepOriginalFeature <code>true</code> in order to keep the original nominal
     *                            feature, <code>false</code> to remove it (default setting).
     * @param dense               <code>true</code> to create zero-values explicitly,
