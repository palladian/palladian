...
public class FloatVectorUtilTest {
    ...
    @Test
    public void testAdd() {
        assertEquals(Arrays.toString(new float[]{1, 2, 3}), Arrays.toString(FloatVectorUtil.add(new float[]{0, 1, 2}, new float[]{1, 1, 1})));
    }

    @Test
    public void testMagnitude() {
        assertEquals(Math.sqrt(2 * 2 + 5 * 5 + -8 * -8 + 2 * 2), FloatVectorUtil.magnitude(new float[]{2, 5, -8, 2, 0}), 0.0001);
    }
