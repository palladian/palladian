...
public class AddressTaggerTest {
    ...
    public void testAddressTagger() throws FileNotFoundException {
        ...
        locationAnnotations = addressTagger.getAnnotations("According to court documents, Welch went to a residence in the 300 block of East Elm Street about 1:45 a.m.");
        assertEquals(1, locationAnnotations.size());
        assertEquals("East Elm Street", locationAnnotations.get(0).getValue());

        locationAnnotations = addressTagger.getAnnotations(
                "Welch and Gillen’s mother, who had accompanied him to the residence, then left in a pickup truck but pulled over near Interstate 435 and 23rd Street and called 911.");
        ...
        locationAnnotations = addressTagger.getAnnotations(
                "Actress and model Elizabeth Hurley will be at Bicester Village, in Pingle Drive, on Thursday to open her new beach boutique at the designer outlet shopping centre.");
        assertEquals("Pingle Drive", locationAnnotations.get(0).getValue());

        locationAnnotations = addressTagger.getAnnotations("Nikki Lynn Barlow, 18, of 208 Routon St. was arrested by PPD Patrolman Amanda Forrest.");
        assertEquals("208", locationAnnotations.get(0).getValue());
        assertEquals(LocationType.STREETNR, locationAnnotations.get(0).getLocation().getType());
        assertEquals("Routon St.", locationAnnotations.get(1).getValue());
        assertEquals(LocationType.STREET, locationAnnotations.get(1).getLocation().getType());

        locationAnnotations = addressTagger.getAnnotations("The incident occurred Sunday in the parking lot of the Price Chopper at Vivion Road and North Oak Trafficway.");
        ...
        locationAnnotations = addressTagger.getAnnotations("An attempt is being made to clean and maintain the trail that circles the reservoir on Shadyside Drive.");
        assertEquals("Shadyside Drive", locationAnnotations.get(0).getValue());

        locationAnnotations = addressTagger.getAnnotations(
                "Board and commission members will have an orientation and training session at 6:30 p.m. Monday, March 30 in Talla 2 at the Dublin Community Recreation Center, 5600 Post Road, Chinnici-Zuercher said.");
        ...
        locationAnnotations = addressTagger.getAnnotations(
                "The 21.5-inch iMacs came with a 3.06GHz Intel Core 2 Duo processor, 4GB of RAM, 1920 x 1080 resolution, and a SuperDrive.");
        assertEquals(0, locationAnnotations.size());
    }
