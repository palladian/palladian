...
public class DateRaterHelperTest {
    ...
    public void testIsDateInRange() {
        ...
        ExtractedDate date = DateParser.parse("2010-01-01T12:30:30Z", RegExp.DATE_ISO8601_YMD_T[1]);
        assertTrue(DateRaterHelper.isDateInRange(date));
        date = DateParser.parse("1990-11-13T00:00:00Z", RegExp.DATE_ISO8601_YMD_T[1]);
        assertTrue(DateRaterHelper.isDateInRange(date));
        ...
        assertTrue(DateRaterHelper.isDateInRange(date));
        date = DateParser.parse("1990-11-12T00:00:00Z", RegExp.DATE_ISO8601_YMD_T[1]);
        assertFalse(DateRaterHelper.isDateInRange(date));
        date = DateParser.parse("2090-11-12T00:00:00Z", RegExp.DATE_ISO8601_YMD_T[1]);
        assertFalse(DateRaterHelper.isDateInRange(date));
        date = DateParser.parse("Nov 8, 2007", RegExp.DATE_USA_MMMM_D_Y[1]);
        assertTrue(DateRaterHelper.isDateInRange(date));
        date = DateParser.parse("3.9.2010", RegExp.DATE_EU_D_MM_Y[1]);
        assertTrue(DateRaterHelper.isDateInRange(date));
        date = DateParser.parse("2010-09", RegExp.DATE_ISO8601_YM[1]);
        assertTrue(DateRaterHelper.isDateInRange(date));
