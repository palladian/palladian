...
public final class SearcherFactory {
    ...
    public static <S extends Searcher<? extends WebContent>> S createSearcher(Class<S> searcherType, Configuration config) {
        ...
        for (Constructor<?> constructor : searcherType.getConstructors()) {
            ...
            if (parameterTypes.length == 1 && parameterTypes[0].equals(Configuration.class)) {
                try {
                    ...
                    if (e.getCause() != null) {
                        throw new IllegalStateException("Could not instantiate " + searcherType.getName() + ", exception from constructor: " + e.getCause().getMessage(),
                                e.getCause());
                    }
                    throw new IllegalStateException("Could not instantiate " + searcherType.getName() + " using the constructor with Configuration: " + e.getMessage(), e);
                } catch (Exception e) {
                    throw new IllegalStateException("Could not instantiate " + searcherType.getName() + " using the constructor with Configuration: " + e.getMessage(), e);
                }
                break;
        ...
        if (searcher == null) {
            try {
                ...
                throw new IllegalStateException("Could not instantiate " + searcherType.getName() + " using the default constructor.");
            }
        }
