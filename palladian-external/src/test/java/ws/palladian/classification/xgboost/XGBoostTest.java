...
import ws.palladian.classification.xgboost.MCCEvaluation;
...
public class XGBoostTest {
	...
	public void simpleTest_twoClass() {
		...
		CategoryEntries categoryEntries = new XGBoostClassifier().classify(new InstanceBuilder().set("feature", 1).create(), model);
	...
	public void testHeart() throws FileNotFoundException {
		TrainTestSplit heartDataset = getHeartDataset();
		...
		assertTrue("accuracy should be greater 0.98, but was " + confusionMatrix.getAccuracy(), confusionMatrix.getAccuracy() > 0.98);
	}
	@Test
	public void testHeart_MCC() throws FileNotFoundException {
		TrainTestSplit heartDataset = getHeartDataset();
		...
		XGBoostModel model = new XGBoostLearner(params, /* rounds */ 10, new MCCEvaluation()).train(heartDataset.getTrain());
	...
	private static TrainTestSplit getHeartDataset() throws FileNotFoundException {
