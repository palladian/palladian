...
public class ProbabilityCalibratedClassifier<M extends Model> implements Classifier<M> {
    ...
    public static <M extends Model> ProbabilityCalibratedClassifier<M> build(M model, Classifier<M> classifier, Dataset train) {
        ...
        for (Instance instance : train) {
            ...
            trainingInstances.add(new InstanceBuilder().set(TRUE_CLASS_PROBABILITY, probability).create(instance.getCategory()));
        }
    ...
    public CategoryEntries classify(FeatureVector featureVector, M model) {
        ...
        return new LibLinearClassifier().classify(new InstanceBuilder().set(TRUE_CLASS_PROBABILITY, probability).create(), calibrationModel);
    }
