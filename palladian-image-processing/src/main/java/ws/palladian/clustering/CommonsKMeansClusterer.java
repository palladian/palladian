...
public final class CommonsKMeansClusterer implements Clusterer {
    ...
    public Collection<Cluster> cluster(Iterable<double[]> vectors) {
        List<Clusterable> clusterables = StreamSupport.stream(vectors.spliterator(), false).map(v -> {
            return new DoublePoint(v);
        }).collect(toList());
        ...
        return clusters.stream().map(c -> {
            return new ImmutableCluster(c.getCenter().getPoint());
        }).collect(toList());
    }
