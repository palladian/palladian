//package ws.palladian.classifiers.transferlearning.playground;
//
//import java.io.File;
//
...
//import ws.palladian.dataset.DatasetIterator;
//
///**
...
//        FineTuneConfiguration fineTuneConf = new FineTuneConfiguration.Builder().activation(Activation.LEAKYRELU).weightInit(WeightInit.RELU).learningRate(5e-5)
//                .optimizationAlgo(OptimizationAlgorithm.STOCHASTIC_GRADIENT_DESCENT).updater(Updater.NESTEROVS).dropOut(0.5).seed(seed).build();
//
...
//        ComputationGraph vgg16Transfer = new TransferLearning.GraphBuilder(vgg16).fineTuneConfiguration(fineTuneConf).setFeatureExtractor(featureExtractionLayer) // "block5_pool"
//                                                                                                                                                                  // and below are
//                                                                                                                                                                  // frozen
...
//                        new OutputLayer.Builder(LossFunctions.LossFunction.NEGATIVELOGLIKELIHOOD).activation(Activation.SOFTMAX).nIn(256).nOut(numClasses).build(), "fc3") // add in
//                                                                                                                                                                           // a
//                                                                                                                                                                           // final
//                                                                                                                                                                           // output
//                                                                                                                                                                           // dense
...
//
//        // Dataset iterators
//        DatasetIterator.setup(batchSize, trainPerc);
