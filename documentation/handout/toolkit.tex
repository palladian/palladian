...
System.out.println("finished training and testing classifier combinations in " + stopWatch.getElapsedTimeString());
\end{lstlisting}
\end{codelisting}
...
It is often interesting which words describe a given text most. These words are often called ``keywords'' or ``tags''. TOOLKIT is able to use a weighted, controlled vocabulary and perform tagging of a text using TF-IDF and tag correlations.

The ControlledTaggerIndex is the basis for the tagging. This index contains information about the controlled tag vocabulary, tag frequencies, a stem-map, and a correlation table for all tags. The tag index is built using training data of single text files that have been assigned with weighted tags. This index is serialized as a model file for later usage.

To tag a new text with the most relevant keywords, the ControlledTagger tokenizes the input text and stems the tokens using the SnowballStemmer. Tokens are now in their stemmed form, that is, tokens such as ``blogging'' and ``blogs'' became ``blog''.

All tokens that match a tag from the controlled vocabulary in the index are held in a bag-of-words which also stores the frequencies of the tokens in the input text. Using this data, we can calculate the TF-IDF values for each tag candidate. The TF-IDF scores are used to rank the tags initially. Another ranking step is performed by using the tag correlation table from the index.

The ControlledTagger can be configured using the ControlledTaggerSettings class. You can configure the following behaviors:
\begin{enumerate}
...
The TOOLKIT wraps 8 Named Entity Recognizers using a single interface making it easier to test and substitute them in real world applications.
...
// desired output
// The <PHONE>iphone 4</PHONE> is the successor of the <PHONE>iphone 3gs</PHONE>.
\end{lstlisting}
\end{codelisting}
...
Many tagging types have been developed which makes interoperability between different systems harder. TOOLKIT can handle many of these formats and is able to transform a tagged text within these formats. 
...
\begin{verbatim}
Bill/PERSON Gates/PERSON founded Microsoft/ORGANIZATION in 1975/DATE.
\end{verbatim}
...
\subparagraph{XML} The XML notation is very common too and is the favored one in TOOLKIT. Similarly to the brackets notation, the XML notation wraps the word in an XML tag. The sample tags would look as shown below.

\begin{verbatim}
<PERSON>Bill Gates</PERSON> founded <ORGANIZATION>Microsoft</ORGANIZATION> in <DATE>1975</DATE>.
\end{verbatim}
...
\begin{figure}[ht!]
\centering
\includegraphics[width=\columnwidth]{img/taggingFormats.pdf}
...
\hline
IllinoisLbjNER	& Path to a configuration file, in this file you can specify which features and parameters should be used and where the trained model should be saved &	data/models/illinoisner/training/baselineFeatures.config \\
\hline
JulieNER	& Output path for the trained model. You can add a further parameter with the path to a configuration file, in this file you can specify which features and parameters should be used &	data/models/juliener/phoneModel.mod and [data/models/juliener/tutorial/featconfig.conf] \\
\hline
LingPipeNER	& Output path for the trained model &	data/models/lingpipe/phoneModel.model \\
\hline
OpenNLPNER	& Output path for the trained model, the filename must follow the format ``openNLP\_TAG.bin.gz'' since you can only train one tag per model &	data/models/opennlp/openNLP\_phone.bin.gz \\
\hline
StanfordNER	& Path to a configuration file, in this file you can specify which features and parameters should be used and where the trained model should be saved &	data/models/lingpipe/training/austen.prop \\
\hline
TUDNER	& Output path for the trained model &	data/models/tudner/phoneModel.model \\
\hline
\end{tabular} 
\label{tab:nerConfig}
\end{table}
...
String successfulOutput = "successful";
if (!successful) {successfulOutput = "not successful";}
System.out.println("Training of " + ner.getName() + " was " + successfulOutput);
\end{lstlisting}
\end{codelisting}
...
\begin{verbatim}
Unlike <PERSON>Robert</PERSON>, <PERSON>John Briggs Jr</PERSON> contacted <ORGANIZATION>Wonderful Stockbrockers Inc</ORGANIZATION> in <LOCATION>New York</LOCATION> and instructed them to sell all his shares in <ORGANIZATION>Acme</ORGANIZATION>.
\end{verbatim}

Furthermore, let us assume that a NER system created the following markup \cite{nadeau2007yooname} for the same text:

\begin{verbatim}
<LOCATION>Unlike</LOCATION> Robert, <ORGANIZATION>John Briggs Jr</ORGANIZATION> contacted Wonderful <ORGANIZATION>Stockbrockers</ORGANIZATION> Inc <DATE>in New York</DATE> and instructed them to sell all his shares in <ORGANIZATION>Acme</ORGANIZATION>.
\end{verbatim}

The only correct match between the correct solution and the NER system output is $<ORGANIZATION>Acme</ORGANIZATION>$, all other markups are some kind of errors.
...
<ORGANIZATION>Wonderful Stockbrockers Inc</ORGANIZATION>	&	<ORGANIZATION>Stockbrockers</ORGANIZATION>	&	The system tagged the entity but the boundaries are wrong.\\
...
System.out.println("F1 Exact Match: " + eResult.getF1(EvaluationResult.EXACT_MATCH));
System.out.println("F1 MUC: " + eResult.getF1(EvaluationResult.MUC));
\end{lstlisting}
\end{codelisting}

\subsection{Web Page Age Detection}
The age of a web page can often be a useful indicator about the freshness of the information. It might also be used in classification or ranking of web documents. TOOLKIT is able to detect the age of many web pages by using four main techniques:
\begin{enumerate}
...
\item Recognize a date in the {\textbf URL} of the web page. Especially blogs use the URL style of ``http://domain.tld/posts/YYYY/MM/PostName'' or similar. In these cases the date in the domain is a very good indicator of the web page's age.
...
TOOLKIT can handle many FAQs that follow the first or third structure. First, the algorithm detects the XPaths to the questions and then it tries to extract all content between the current and the next question as the answer for the current question.
...
seedAttributes.add(new Attribute("Second camera", attributeType, concept));
seedAttributes.add(new Attribute("Memory card", attributeType, concept));
seedAttributes.add(new Attribute("Form factor", attributeType, concept));
...
The TOOLKIT makes excessive use of third party libraries. We do not intend to re-implement code but rather to built on it and create something superior. Here an incomplete list of libraries the toolkit uses:
\begin{itemize}
