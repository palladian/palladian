...
public class EvaluationHelper {
    ...
    public static Map<String, DBExport> readFile(int entries, boolean random, String dataset) {
        ...
        try {
            ...
            String line;
            int lineIndex = 0;
            int urlIndex = 0, pathIndex = 0, pubDateIndex = 0, pubSurenessIndex = 0, modDateIndex = 0, modsurenessIndex = 0, googleIndex = 0, hakiaIndex = 0, askIndex = 0, headerLastModIndex = 0, headerDateIndex = 0, downIndex = 0;
            String[] parts;
            while ((line = br.readLine()) != null && (lineIndex <= maxEntries || maxEntries == -1)) {
                ...
                if (lineIndex == 0) {
                    ...
                    for (int i = 0; i < parts.length; i++) {
                        set.put(parts[urlIndex], new DBExport(parts[urlIndex], parts[pathIndex], parts[pubDateIndex], parts[modDateIndex], Boolean.valueOf(parts[pubSurenessIndex]),
                                Boolean.valueOf(parts[modsurenessIndex]), parts[googleIndex], parts[hakiaIndex], parts[askIndex], parts[headerLastModIndex], parts[headerDateIndex],
                                parts[downIndex]));
                    }
                }
                lineIndex++;
