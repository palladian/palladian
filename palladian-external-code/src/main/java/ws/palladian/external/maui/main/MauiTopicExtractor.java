package ws.palladian.external.maui.main;
...
import ws.palladian.external.maui.stemmers.*;
import ws.palladian.external.maui.stopwords.*;
...
public class MauiTopicExtractor implements OptionHandler {
	...
	public void setOptions(String[] options) throws Exception {
		...
		if (stopwordsString.length() > 0) {
			stopwordsString = "ws.palladian.external.maui.stopwords.".concat(stopwordsString);
		...
		if (stemmerString.length() > 0) {
			stemmerString = "ws.palladian.external.maui.stemmers.".concat(stemmerString);
