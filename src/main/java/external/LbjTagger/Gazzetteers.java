...
public class Gazzetteers{
		...
		for(int i=0;i<filenames.size();i++)
		{
			...
			while(line!=null){
				...
				while(st.hasMoreTokens()){
					String s=external.lbj.StringStatisticsUtils.MyString.cleanPunctuation(st.nextToken());
	...
	public static void annotate(NEWord w)
	{
		...
		{
			if(dictionariesOneWordIgnorePunctuation.elementAt(j).containsKey(external.lbj.StringStatisticsUtils.MyString.cleanPunctuation(w.form))){
