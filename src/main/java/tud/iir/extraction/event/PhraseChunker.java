...
public class PhraseChunker implements Chunker {
	...
	public PhraseChunker(HmmDecoder posTagger, TokenizerFactory tokenizerFactory) {
		mPosTagger = posTagger;
		mTokenizerFactory = tokenizerFactory;
	}

	public Chunking chunk(CharSequence cSeq) {
		final char[] cs = Strings.toCharArray(cSeq);
	...
	public Chunking chunk(char[] cs, int start, int end) {
		...
		final Tokenizer tokenizer = mTokenizerFactory.tokenizer(cs, start, end
				- start);
		...
		// part-of-speech tag
		final Tagging<String> tagging = mPosTagger.tag(tokenList);
		...
		for (int i = 0; i < tagging.size();) {
			...
			if (START_NOUN_TAGS.contains(tagging.tag(i))) {
				...
				Chunk chunk = ChunkFactory.createChunk(startChunk, trimmedEndChunk,
						"noun");
				chunking.add(chunk);
				startChunk = endChunk;
	...
	public static void main(String[] args) {
		...
		// parse input params
		final File hmmFile = new File(args[0]);
		final int cacheSize = Integer.valueOf(args[1]);
		...
		try {
			posHmm = (HiddenMarkovModel) AbstractExternalizable
					.readObject(hmmFile);
		} catch (IOException e) {
		...
		// construct chunker
		final HmmDecoder posTagger = new HmmDecoder(posHmm, null, cache);
		final TokenizerFactory tokenizerFactory = IndoEuropeanTokenizerFactory.INSTANCE;
		final PhraseChunker chunker = new PhraseChunker(posTagger, tokenizerFactory);
		
		// apply chunker & pos tagger
		for (int i = 2; i < args.length; ++i) {
			LOGGER.trace("\n" + args[i]);
			String[] tokens = tokenizerFactory.tokenizer(args[i].toCharArray(),
					0, args[i].length()).tokenize();
