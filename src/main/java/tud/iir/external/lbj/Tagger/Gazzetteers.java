package tud.iir.external.lbj.Tagger;
		...
		for(int i=0;i<filenames.size();i++)
		{
			...
			while(line!=null){
				...
				while(st.hasMoreTokens()){
                    String s = tud.iir.external.lbj.StringStatisticsUtils.MyString.cleanPunctuation(st.nextToken());
	...
	public static void annotate(NEWord w)
	{
	       	...
	       	for(int j=0;j<dictionaries.size();j++)
		{
            if (dictionariesOneWordIgnorePunctuation.elementAt(j).containsKey(
                    tud.iir.external.lbj.StringStatisticsUtils.MyString.cleanPunctuation(w.form))) {
