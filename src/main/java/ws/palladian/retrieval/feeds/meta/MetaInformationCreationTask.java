...
public final class MetaInformationCreationTask implements Runnable {
	...
	private final static Logger LOGGER = Logger
			.getLogger(MetaInformationCreator.class);
	...
	public void run() {
		...
		try {
			...
			ConnectionTimeoutPool timeoutPool = ConnectionTimeoutPool
					.getInstance();
			...
			throw new RuntimeException("URL of feed with id: " + feed.getId()
					+ " is malformed!", e);
		} catch (IOException e) {
			throw new RuntimeException("Could not reed from feed with id: "
					+ feed.getId(), e);
		}
		...
		try {
			...
			LOGGER.error("Unable to check if feed at URL " + feed.getFeedUrl()
					+ " is accessible.");
		}
		...
		try {
			...
			LOGGER.error("Could not get HTTP header information for feed with id: "
					+ feed.getId() + ".");
		}
		...
		LOGGER.info("percent done: "
				+ MathHelper.round(100 * MetaInformationCreator.counter
						/ (double) MetaInformationCreator.collectionSize, 2)
				+ "(" + MetaInformationCreator.counter + ")");
	}
	...
	private Boolean isAccessibleFeed(HttpURLConnection connection)
			throws IOException {
		if (HttpURLConnection.HTTP_NOT_FOUND == connection.getResponseCode()
				|| HttpURLConnection.HTTP_FORBIDDEN == connection
						.getResponseCode()) {
			return false;
		}
	...
	private Boolean getFeedSupportsPubSubHubBub(final URLConnection connection)
			throws IOException {
		if (currentFeedContent != null
				&& currentFeedContent.contains("rel=\"hub\"")) {
	...
	private Integer getFeedResponseSize(final HttpURLConnection connection) {
		int ret = 0;
		try {
			if (HttpURLConnection.HTTP_NOT_MODIFIED == connection
					.getResponseCode()) {
				ret = new Integer((connection.getContentLength() == -1 ? 0
	 ...
	 * @return {@code true} if the feed supports conditional gets, {@code false}
	 *         otherwise.
	 * @throws IOException
	 */
	private Boolean getFeedSupports304(final HttpURLConnection connection)
			throws IOException {
		Boolean ret = false;
