 ...
 * 
 */
public class DateEvaluator_old {
	...
	private ContentDateRater_old cdr;
	 ...
	 * Standard constructor.
	 */
	public DateEvaluator_old(PageDateType pub_mod) {
		setPubMod(pub_mod);
	}
	 ...
	 * @param url
	 */
	public DateEvaluator_old(String url, PageDateType pub_mod) {
		this.url = url;
		setPubMod(pub_mod);
	}
	 ...
	 */
	public DateEvaluator_old(String url, boolean referenceLookUp,
			PageDateType pub_mod) {
		this.url = url;
	...
	private void setPubMod(PageDateType pub_mod) {
		...
		cdr = new ContentDateRater_old(pub_mod);
	 ...
	 * Coordinates all rating techniques and deploying of rates to lower
	 * techniques.
	 * 
	 * @param <T>
	...
	public <T> HashMap<T, Double> rate(ArrayList<T> extractedDates) {
		...
		ArrayList<MetaDate> metaDates = (ArrayList<MetaDate>) DateArrayHelper
				.filter(dates, DateType.MetaDate);


		ArrayList<StructureDate> structDates = (ArrayList<StructureDate>) DateArrayHelper
				.filter(dates,DateType.StructureDate);

		ArrayList<ContentDate> contDates = (ArrayList<ContentDate>) DateArrayHelper
				.filter(dates, DateType.ContentDate);
		ArrayList<ContentDate> contFullDates = (ArrayList<ContentDate>) DateArrayHelper
				.filter(contDates, DateArrayHelper.FILTER_FULL_DATE);

		ArrayList<ArchiveDate> archiveDate = (ArrayList<ArchiveDate>) DateArrayHelper
				.filter(dates, DateType.ArchiveDate);

		ArrayList<ReferenceDate> referenceDate = (ArrayList<ReferenceDate>) DateArrayHelper
				.filter(dates,DateType.ReferenceDate);

		if (urlDates != null && urlDates.size() > 0) {
			urlResult.putAll((Map<? extends T, ? extends Double>) udr
					.rate(urlDates));
		}
		...
		if (contFullDates != null && contFullDates.size() > 0) {
			contResult.putAll((Map<? extends T, ? extends Double>) cdr
					.rate(contFullDates));
		...
		if (urlResult.size() > 0 && contResult.size() > 0) {
			checkDayMonthYearOrder(DateArrayHelper.getFirstElement(urlResult),
					contResult);
		}

		if (structDates != null && structDates.size() > 0) {
			structResult.putAll((Map<? extends T, ? extends Double>) sdr
					.rate(structDates));
		}
		...
		evaluatedDates.putAll(deployStructureDates(contResult,
				(HashMap<StructureDate, Double>) structResult));
		...
		if (archiveDate != null && archiveDate.size() > 0) {
			evaluatedDates.putAll((Map<? extends T, ? extends Double>) adr
					.rate(archiveDate, evaluatedDates));
		}
	 ...
	 * 
	 */
	public <T> void checkDayMonthYearOrder(T orginalDate,
			HashMap<T, Double> toCheckcDates) {
		if (orginalDate != null) {
			for (Entry<T, Double> e : toCheckcDates.entrySet()) {
				if (e.getKey() != null) {
					DateRaterHelper.checkDayMonthYearOrder(orginalDate,
							(ExtractedDate) e.getKey());
				}
	...
	/**
	 * Method with calculations for new rating of dates by consideration of
	 * meta-dates like HTTP and Head.
	 * 
	 * @param <T>
	 * @param metaDates
	 *            HashMap with HTTP or head-Dates.
	 * @param dates
	 *            Dates to be rated.
	 * @return
	 */
	public static <T> HashMap<T, Double> deployMetaDates(
			HashMap<T, Double> metaDates, HashMap<T, Double> dates) {
		...
		HashMap<T, Double> tempContentDates = new HashMap<T, Double>(); // only
																		...
																		// to
																		// metaDate.
		HashMap<T, Double> tempResult = new HashMap<T, Double>(); // worked
																	// dates can
																	// be put
																	// in.

		Entry<T, Double>[] orderedMetaDates = DateArrayHelper.orderHashMap(
				metaDates, true);
		for (int stopcounter = 0; stopcounter < 3; stopcounter++) {
			...
			for (int i = 0; i < orderedMetaDates.length; i++) {
				...
				// DateComparator.STOP_MINUTE instead of stopFlag, because
				// original dates should be distinguished up to
				// minute.
				int countFactor = DateArrayHelper.countDates(metaDate,
						metaDates, DateComparator.STOP_MINUTE) + 1;
	 ...
	 * @return
	 */
	private <T> HashMap<T, Double> deployURLDate(HashMap<T, Double> urlDates,
		...
		for (Entry<T, Double> url : urlDates.entrySet()) {
			...
			/**
			 * temp = DateArrayHelper.getDifferentDatesMap(urlDate, dates,
			 * urlDate.getExactness()); for (Entry<T, Double> date :
			 * temp.entrySet()) { double newRate = date.getValue() - (0.2 *
			 * date.getValue() * (urlRate * urlFactor));
	 ...
	 * position in document and age of date.
	 * 
	 * @param dates
	 * @return New rated dates.
	 */
	private HashMap<ContentDate, Double> guessRate(
		...
		if (result.size() > 0 && result != null) {
			...
			int maxPos = orderPosInDoc.get(orderPosInDoc.size() - 1).get(
					ContentDate.DATEPOS_IN_DOC);
			int counter = 0;
	 ...
	 * @return
	 */
	private <C> HashMap<C, Double> deployStructureDates(
