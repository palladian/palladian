 ...
 * For more than one use, use {@link ContentDateGetter#reset()} to avoid errors
 * and mistakes. <br>
...
public class ContentDateGetter extends TechniqueDateGetter<ContentDate> {
	...
	private void setFeatures(ArrayList<ContentDate> dates) {
		...
		for (ContentDate date : dates) {
			...
			if (metaDates.size() > 0
					&& DateArrayHelper.countDates(date, metaDates,
							DateComparator.STOP_DAY) > 0) {
				date.setInMetaDates(true);
			}
			if (urlDates.size() > 0
					&& DateArrayHelper.countDates(date, urlDates,
							DateComparator.STOP_DAY) > 0) {
				date.setInUrl(true);
			}
			...
			if (datePosOrderAbsl > 0) {
				date.setDistPosBefore(date.get(ContentDate.DATEPOS_IN_DOC)
						- posOrder.get(datePosOrderAbsl - 1).get(
								ContentDate.DATEPOS_IN_DOC));
			}
			if (datePosOrderAbsl < posOrder.size() - 1) {
				date.setDistPosAfter(posOrder.get(datePosOrderAbsl + 1).get(
						ContentDate.DATEPOS_IN_DOC)
			...
			if (dateAgeOrdAbsl > 0) {
				date.setDistAgeBefore(Math.round(dc.getDifference(date,
						ageOrder.get(dateAgeOrdAbsl - 1),
						DateComparator.MEASURE_HOUR)));
			}
			if (dateAgeOrdAbsl < ageOrder.size() - 1) {
				date
						.setDistAgeAfter(Math.round(dc.getDifference(date,
								ageOrder.get(dateAgeOrdAbsl + 1),
								DateComparator.MEASURE_HOUR)));
			}
		}
	...
	private ArrayList<ContentDate> getContentDates(Document document) {
		...
		if (!nodeList.isEmpty()) {
			...
			this.doc = StringHelper.removeDoubleWhitespaces(HTMLHelper
					.replaceHTMLSymbols(HTMLHelper.documentToReadableText(body
							.item(0))));
			...
			for (int i = 0; i < nodeList.size(); i++) {
				...
				if (nodeList.get(i).getNodeType() == Node.TEXT_NODE) {
					...
					if (parent.getNodeType() != Node.COMMENT_NODE
						...
						dates.addAll(checkTextnode((Text) node, matcher,
								regExps));
					}
				}
			}
	 ...
	 * @return
	 */
	private ArrayList<ContentDate> checkTextnode(Text node, Matcher[] matcher,
			Object[] regExps) {

		String text = StringHelper.removeDoubleWhitespaces(HTMLHelper
				.replaceHTMLSymbols(node.getNodeValue()));
		int index = -1;
		Node parent = node.getParentNode();
		Node tag = parent;
		...
		for (String textPart : textSplitt) {
			dateList.addAll(DateGetterHelper.findALLDates(textPart, matcher,
					regExps));
		}
		if (dateList.size() > 0) {
		...
		for (ContentDate date : dateList) {
			...
			if (date.getStructureDate() == null && tag != parent) {
				date
						.setStructureDate(getStructureDate(parent, matcher,
								regExps));
			}
			...
			if (index != -1) {
				int ablsDocPos = index
						+ date.get(ContentDate.DATEPOS_IN_TAGTEXT);
			...
			if (keyword.equals("") || keyword3Class) {
				...
				if (date.getKeyword() != null) {
					date.set(ContentDate.KEYWORDLOCATION,
							ContentDate.KEY_LOC_CONTENT);
					date.setKeyLoc("2");
					date.setKeyLoc202("1");
	...
	private void setClosestKeyword(ContentDate date) {
		...
		if (datePos >= 0) {
			...
			if (keyword != null) {
				date.setKeyword(keyword);
				int diff = StringHelper.countWhitespaces(this.doc.substring(
						subStart, subEnd));
	...
	private StructureDate findStructureDate(Node node, Matcher[] matcher,
		...
		for (int i = 0; i < attributes.getLength(); i++) {
			...
			if (!attr.getNodeName().equalsIgnoreCase("href")) {
				date = DateGetterHelper.findDate(attr.getNodeValue(), matcher,
						regExps);
				if (date != null) {
